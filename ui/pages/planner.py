...
def render(session):
    ...
    st.caption("Enter monthly income, add recurring bills, and visualize what remains for routing.")
    ...
    with st.container(border=True):
        ...
        pay_frequency = c2.selectbox("Pay frequency", ["weekly", "biweekly", "monthly"], index=["weekly", "biweekly", "monthly"].index(profile.pay_frequency if profile.pay_frequency in ["weekly", "biweekly", "monthly"] else "monthly"))
        if st.button("Save Income", type="primary"):
            save_income_profile(session, monthly_income, pay_frequency)
    ...
    with st.container(border=True):
        ...
        due_day = c3.number_input("Due day", min_value=1, max_value=31, step=1)
        c4, c5 = st.columns(2)
        category = c4.text_input("Category", value="Utilities")
        autopay = c5.checkbox("Autopay")
        if st.button("Save Bill") and name:
            add_bill(session, name, amount, due_day, category, autopay)
    ...
    m2.metric("Total Bills", f"${summary['total_bills']:.2f}")
    m3.metric("Autopay", f"${summary['autopay_total']:.2f}")
    m4.metric("Remaining", f"${summary['remaining']:.2f}")
    ...
    if not bills_df.empty:
        ...
        st.dataframe(bills_df.sort_values(["due_day", "bill"]), use_container_width=True)
    else:
        st.info("No bills yet. Add bills to unlock automation and projections.")

    if st.button("Generate This Month's Bill Tasks"):
